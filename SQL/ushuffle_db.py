...
def connect(db):
    ...
    if db == "sqlite":
        ...
        if not os.path.isdir(dbDir):
            os.mkdir(dbDir)
        cxm = sqlite3.connect(os.path.join(dbDir, DBNAME))
    elif db == "mysql":
        try:
            import MySQLdb
...
def insert(cur, db):
    if db == "sqlite3":
        cur.executemany("INSERT INTO users VALUSE(?, ?, ?)",
        ...
        print "INSERT INTO user VALUES(%s, %s, %s)", \
            [(who, uid, rand(1, 5)) for who, uid in randName()][0]
